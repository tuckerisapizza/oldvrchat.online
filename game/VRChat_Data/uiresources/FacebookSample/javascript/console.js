(function(window, $, engine, _) {
	...
	var Output = function (gameConsole) {
		this.gameConsole = gameConsole;
		this.self = this;
	};
	
	Output.prototype.callback = function (result) {
		this.gameConsole.appendResult(result);
		this.self = null;
	};
	
	Output.prototype.error = function (error) {
		this.gameConsole.appendError(error);
		this.self = null;
	};
	...
	var gameConsole = {
		...
		trace : function() {
			this.append(JSON.stringify(arguments), this._traceTemplate);
		},
		...
		_engineCommand : function (name) {
			var output = new this.Output(this);
			return function (gameConsole, name, line) {
				engine.call('gameConsole:' + name, line, output);
			};
		},
